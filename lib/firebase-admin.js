...
for (let i = 0; i < Math.max(fullPrivateKey.length, testKey1.length); i++) {
  if (fullPrivateKey[i] !== testKey1[i]) {
    console.log(`Difference at index ${i}:`);
    console.log(`fullPrivateKey: ${fullPrivateKey[i] || "undefined"}`);
    console.log(`testKey1: ${testKey1[i] || "undefined"}`);
    break; // Stop after the first difference for brevity
  }
}
serviceAccount = {
